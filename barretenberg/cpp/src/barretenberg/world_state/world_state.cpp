...
{
    StateReference current_state_ref = get_state_reference(revision);
    std::vector tree_ids{
        MerkleTreeId::NULLIFIER_TREE,
        MerkleTreeId::NOTE_HASH_TREE,
        MerkleTreeId::PUBLIC_DATA_TREE,
    ...
    return std::all_of(tree_ids.begin(), tree_ids.end(), [=](auto id) {
        if (state_ref.at(id) != current_state_ref.at(id)) {
            std::cout << "State reference mismatch for tree " << getMerkleTreeName(id)
                      << " expected: " << state_ref.at(id).first << " " << state_ref.at(id).second
                      << " got: " << current_state_ref.at(id).first << " " << current_state_ref.at(id).second
                      << "\n\n\n\n";
        }
        return state_ref.at(id) == current_state_ref.at(id);
    });
    // return current_state_ref == state_ref;
}
